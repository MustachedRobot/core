...
class Controller_Admin extends \Controller_Admin
{
     ...
     * 
     * @param String $start (yyyy-mm-dd) Start of the analytics range (optionnal,  default = 30 days from now)
     * @param string $end   (yyyy-mm-dd) End of the analytics range (optionnal, default = today)
     */
    public function action_stats($start = null, $end = null)
    {
        ...
        foreach ($checkins as $checkin) {
            ...
            $checkinperday[$checkin_date] = isset($checkinperday[$checkin_date]) ? $checkinperday[$checkin_date]++ : 1;

        }

        // Add empty dates and count the number of "opened" days (coworking considered as open when at least one person has checked in)
        ...
        $this->data['opened_days'] = $openedDays;
